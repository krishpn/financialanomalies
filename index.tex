...
Fama and MacBeth (1973) two-parameter regression model estimates average
risk-return relationships based on efficient market porfolio (\(m\))
constructed according to Equation \ref{Equation:BlackScholes1972} for an
asset (\(i\)).
...
Equation \ref{Equation:BlackScholes1972} refers to the market
equilibrium (market portflio) is always efficient (Black (1972)).

Excepted Return is given by Equation
...
Hence, Equation \ref{Equation:ExpectedReturnFama1953} refers that
expected return on security \(i\) is \(E(\tilde{R_0})\), the expected
return on a security that is riskless in the portfolio \(m\), plus a
risk premium that is \(\beta_i\) times the difference between
\(E(\tilde{R_m})\) and \(E(\tilde{R_0})\)
...
In Equation \ref{Equation:ExpectedReturnFama1953EquationTen} the
indepenent variable \(\tilde{\beta}_{p,t-1}\) is the average of the
