...
Extreme Gradient Boosting (XGBoost) is a highly scalable, powerful and leading decision tree algorithm proposed by Chen and Guestrin (2016). Designed to handle the large datasets, XGBoost implements distributed gradient tree boosting learning strategy to train the model. That is, it sequentially learns from multiple weak classifiers (base learners or decision trees) and iteratively updates their weights (Friedman et al. (2000), Littlestone and Warmuth (1989)). Each combination of the update of the "weak learners" creates the powerful ensemble of "stronger learners". The sequential ordering means tree generated in the later steps will consider the prediction results of the preceding steps. In other words, the strategy is to gradually instruct weak learners to correct the fault from the preceding step. It also accounts for the deviations when comparing between the former and later decision trees of each iteration. In summary, XGBoost trains model in stages as follows:

1.  Initialize model with a single decision tree.

2.  Calculate the residuals of the model for each iteration. Residuals also known as errors are the differences between the actual and predicted values.

3.  Construct and fit a new decision tree (weak learners) to predict the residuals.

4.  Add the decision tree to the ensemble model in a way that minimizes the loss function.

5.  Update the predictions, then added to preceding predictions to effectively updating model's approximation.

6.  Calculate the new residuals based on the updated predictions.

7.  Build a new tree and update the residuals repeated for predefined number of times.

8.  Combine all predictions to give final predictions of the ensemble

Formally, in a given training dataset $\mathbb{D} = (X_i, y_i)$, where $\mathbb{D}$ is organized in $n$ rows and $m$ columns, $X_i$ denotes vector of covariates and $y_i$ represents labels (if lawful = $1$, else $0$), and $|D| =n, x_i \in \mathbb{R}^m$ and $y_i \in \mathbb{R}$. The predicted value of $\hat{y}$ for the $i^{th}$ sample can be represented by a model comprising $k$ decision trees, expressed as in Equation \ref{Equation:xgboostequationone}.

```{=tex}
\begin{align}
    &\hat{y_i} = \sum_{i=1}^K f_k(x_i), f_k \in \mathbfcal{F},
\end{align}
where $f_k$ represents the $k^{th}$ decision tree, $K$ is the number of trees and $\mathbfcal{F}$ is the functional space denoting the collection of all possible decision trees.
\label{Equation:xgboostequationone}
```
In order to predict the tree in subsequent step, the algorithm computes the loss function given by \ref{Equation:xgboostequationtwo}.

```{=tex}
\begin{align}
    &L = \sum_{i=1}^n \ell(y_i, \hat{y_i}),
\end{align}
where $L$ is the loss function, and respectively $y_i$ and $\hat{y}_i$ are the real and  predicted value of sample $i$. 
\label{Equation:xgboostequationtwo}
```
In addition to the loss function , the salient feature of the XGBoost is also to estimate the regularization term. Together loss function and regularization term results into the objective function (see Equation \ref{Equation:xgboostequationtthree}). The model estimates and find the best fitting parameters ($\theta$) of the objective function during training.

```{=tex}
\begin{align}
    &Obj(\theta) = \sum_{i=1}^n \ell(y_i, \hat{y_i}) + \sum_{k=1}^K \Omega(f_k),
\end{align}
where $\theta$ are the parameters to be estimated, $\ell$ is the loss function, respectively $y_i$ and $\hat{y}_i$are the real and  predicted value of sample $i$, and $\Omega$ is the regularization term. 
\label{Equation:xgboostequationtthree}
```
The loss function $\ell$ can be different type, for example, mean squared error, log loss, etc. The regularization $\Omega(f_k)$ component represents the penalty term for $k^{th}$ tree estimated according to Equation \ref{Equation:xgboostequationtfour}

```{=tex}
\begin{align}
    \Omega(f_k) = \gamma T_k + \frac{1}{2} \lambda \sum_{j=1}^{T_k} w_{k,j}^2,
\end{align}
where $w_{k,j}^2$ is the weight of the $j^{th}$ leaf in the $k^{th}$ tree, $T$ is the number of leaf nodes, $\gamma$ and $\lambda$ are respectively minimum loss requires  shrinkage (learning) parameters. 
\label{Equation:xgboostequationtfour}
```
`END`
...
```
@figlewski1981informational estimates "The test methodology we will use is a familiar one. We construct ten port? folios to which stocks are assigned according to their average short interest during the previous six months, and calculate the portfolio a's or excess returns."
...
-   O risk
    ...
    -   CHIN = $({NI}_t-{NI}_{t-1})/(|{NI}_t| + |{NI}_{t-1}|)$ where ${NI}_t$ is net income for the most recent period.
...
@griffin2002book: "This paper examines the relationship between book-to-market equity, distress risk, and stock returns. Among firms with the highest distress risk as proxied by @ohlson1980financial 0-score, the difference in returns between high and low book-tomarket securities is more than twice as large as that in other firms. This large return differential cannot be explained by the three-factor model or by differences in economic fundamentals. Consistent with mispricing arguments, firms with high distress risk exhibit the largest return reversals around earnings announcements, and the book-to-market effect is largest in small firms with low analyst coverage. "
...
```
@hou2005market @nagel2005short @asquith2005short @mohanram2005separating @whited2006financial @ang2006cross @anderson2006empirical @daniel2006market @fama2006profitability @bradshaw2006relation @franzoni2006pension @narayanamoorthy2006conservatism @avramov2007momentum @kumar2008estimation @guo2008average @campbell2008search @garlappi2008default @cooper2008asset @pontiff2008share @cohen2008economic @fabozzi2008sin @soliman2008use @avramov2009dispersion @hahn2005financial @rozeff1988market @lebedeva2012trading @fishman1995mandatory @cao2004does @pagano1996transparency @chemmanur2019advertising @gow2009earnings @dechow1997returns @tuzel2010corporate @menzly2010market @xing2010does @bali2011maxing @yan2011jump @eisfeldt2013organization @callen2013accounting @drake2011should @hafzalla2011percent @landsman2011investors @li2011well @thomas2011tax @johnson2012option @palazzo2012cash @bazdrech2009labor @cohen2012complicated @hirshleifer2013innovative @prakash2013deferred @cohen2013misvaluing @novy2013other @frazzini2014betting @valta2016strategic @da2009cashflow @hou2011factors @barber2001can @harvey2000conditional @spiess1999long @ang2006downside @chordia2006earnings @kishore2008earnings @hawkins1984earnings @adrian2014financial @frankel1998accounting @abarbanell1998abnormal @ali2003arbitrage @vanden2006option @vanden2004options @chan1985exploratory @arbel1983giraffes @campbell1996understanding @dittmar2002nonlinear @liu2006liquidity @pastor2003liquidity @hirshleifer2010financing @black1972capital @carhart1997persistence @black1972capital @kraus1976skewness @lintner1965security @sharpe1964capital @heckerman1972portfolio @heckerman1972portfolio @fama1973risk @mossin1966equilibrium @jagannathan1996conditional @ferson1991variation @fogler1981three @boudoukh2007importance @hou2015digesting @holthausen1992prediction @easley2010factoring @chopra1992measuring @kelly2021understanding @chen2011alternative @foster1984earnings @spiess1995underperformance @daniel1997evidence @fama1993common @fama1992cross @lo2006trading @mayshar1981transaction @brennan1996market @jiang2005information @lev2005r @bali2009volatility @boyer2010expected @an2010warranted @hameed2015industries @simutin2010excess @opler1999determinants @huang2012extreme @balachandran2012using @conrad2013ex @conrad2013ex @burlacu2012risk @durnev2009corporate @vassalou2004default @adrian2008stock @wahlen2011can @penman2002modeling @lev2016informational @cen2006forecasted @baik2007changes @gu2009information @huangalanguoming2009tcso @papanastasopoulosgeorgios2010tior @dazhi2011dsrr

@PatatoukasPanosN.2012CCIf

@mclean2016does

@baltussen2012unknown

@an2014joint @li2010real
...
@cremers2009takeovers

@beneish1997detecting
...
@gokcen2009information

@korniotis2009geography
...
@armstrong2011information

@hess2010projected

@brown2007productivity
...
@balvers2007productivity

@teo2004style

@gu2011Overpriced

@chen2012idiosyncratic
...
@xing2008interpreting

@bansal2005consumption

@parker2005consumption

@brennan2004estimation

@chan1996does

@chapman1997approximating
...
@breeden1979intertemporal

@stulz1986asset
@george2010resolution
@livdan2009Financially
@adler1983International
@rubinstein1973fundamental
@constantinides1982Intertemporal
@rubinstein1974aggregation
@sun2019Financing
@garleanu2012Displacement
@cox2005theory
@lucas1978Asset
@garlappi2011Financial
@jarrow1980Heterogeneous
@merton1973intertemporal
@constantinides1986Capital
@stulz1981model
@solnik1974equilibrium
@chordia2013Trends
@ferson2013sample
@akbas2013Time
@doskov2016Tradable
@jagannathan2007Lazy
@vassalou2003News
@jacobs2004Idiosyncratic
@ozoguz2009Good
@viale2012Safety
@malloy2009Long
@sweeney1986pricing
@kumar2006Retail
@oldfield1981Treasury
@kapadia2011Tracking
@ferson1993risk
@ferson1994Sources
